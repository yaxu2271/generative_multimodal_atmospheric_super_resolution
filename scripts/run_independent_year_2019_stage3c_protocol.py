#!/usr/bin/env python3
"""Run one missing protocol from a frozen 2019 directional 4-cube."""
...
from list_stage3c_protocols import (  # noqa: E402
    DEFAULT_MANIFEST as DEFAULT_CARTESIAN_MANIFEST,
    load_manifest as load_cartesian_manifest,
)
...
def find_protocol(manifest: dict, protocol_id: str) -> dict:
    ...
    if len(matches) != 1:
        raise KeyError(f"Expected one Stage III-C protocol for {protocol_id!r}")
    return matches[0]
...
def main() -> None:
    ...
    parser.add_argument(
        "--cartesian-manifest", type=Path, default=DEFAULT_CARTESIAN_MANIFEST
    )
    ...
    cartesian, cartesian_digest = load_cartesian_manifest(args.cartesian_manifest)
    if cartesian["base_candidate_manifest_sha256"] != base_digest:
        raise ValueError("Stage III-C manifest does not match base manifest")
    if cartesian["selection_artifact_sha256"] != selection_digest:
        raise ValueError("Stage III-C manifest does not match selection artifact")

    protocol = find_protocol(cartesian, args.protocol_id)
    ...
    if args.smoke:
        sampling.update({"ensemble": 1, "steps": 5, "mode": "stage3c_smoke"})
    else:
        sampling["mode"] = "frozen_2019_stage3c_directional_cartesian4"
    resolved = {
        ...
        "cartesian_manifest": str(args.cartesian_manifest.resolve()),
        "cartesian_manifest_sha256": cartesian_digest,
        "protocol": protocol,
