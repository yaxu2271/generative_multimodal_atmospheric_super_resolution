"""Model architectures for diffusion-based generative models."""

import numpy as np
import torch
from torch.nn.functional import silu
from igra_gen.models.abstract import AbstractNetwork
...
class SongUNet(AbstractNetwork):
    def __init__(
        ...
        in_channels: int,
        out_channels: int,
        auxilary_dim=0,  # Number of class auxilarys, 0 = unconditional.
        augment_dim=0,  # Augmentation auxilary dimensionality, 0 = no augmentation.
        model_channels=128,  # Base multiplier for num channels.
        channel_mult=[1, 2, 2, 2],  # Per-resolution multipliers for num channels.
        channel_mult_emb=4,  # Multiplier for the dimensionality of the embedding vector.
        num_blocks=4,  # Number of residual blocks per resolution.
        attn_resolutions=[[0, 0]],  # List of resolutions with self-attention.
        dropout=0.10,  # Dropout probability of intermediate activations.
        auxilary_dropout=0,  # Dropout probability of class auxilarys for classifier-free guidance.
        padding_mode="zeros",  # Default padding mode for convolutions:
        embedding_type="positional",  # Timestep embedding type: 'positional' for DDPM++, 'fourier' for NCSN++.
        channel_mult_noise=1,  # Timestep embedding size: 1 for DDPM++, 2 for NCSN++.
        encoder_type="standard",  # Encoder architecture: 'standard' for DDPM++, 'residual' for NCSN++.
        decoder_type="standard",  # Decoder architecture: 'standard' for both DDPM++ and NCSN++.
        resample_filter=[1, 1],  # Resampling filter: [1,1] DDPM++, [1,3,3,1] NCSN++.
        **kwargs,
    ):
        super().__init__(img_resolution, in_channels, out_channels)

        img_resolution = np.array(img_resolution, dtype=int)
        assert img_resolution.shape[0] == 2
        ...
        self.auxilary_dropout = auxilary_dropout
        ...
        self.map_auxilary = (
            Linear(in_features=auxilary_dim, out_features=noise_channels, **init)
            if auxilary_dim
            else None
        )
        self.map_augment = (
    ...
    def forward(self, x, t, auxilary=None, augment_auxilarys=None):
        ...
        if self.map_auxilary is not None:
            tmp = auxilary
            if self.training and self.auxilary_dropout:
                tmp = tmp * (
                    torch.rand([x.shape[0], 1], device=x.device)
                    >= self.auxilary_dropout
                ).to(tmp.dtype)
            emb = emb + self.map_auxilary(tmp * np.sqrt(self.map_auxilary.in_features))
        if self.map_augment is not None and augment_auxilarys is not None:
            emb = emb + self.map_augment(augment_auxilarys)
